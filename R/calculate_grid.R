...
#' @param learning_rate The rate at which the gradient descent finds the optimum
#' cellsize to ensure that your gridded points fit within the outer boundary of
#' the input polygons.
#' @param grid_type Either 'hexagonal' for a hexagonal grid (default) or
#' 'regular' for a regular grid.
#' @param seed An optional random seed integer to be used for the grid
#' calculation algorithm.
#' @param verbose A logical indicating whether messages should be printed as
...
calculate_grid.SpatialPolygonsDataFrame <- function(shape, learning_rate = 0.03,
  ...
  repeat {
    hex_pts <- sp::spsample(shape, type = grid_type, cellsize = cellsize,
      iter = 10000)
  ...
  # or spatial polygons? need to turn this into same object as hexagons above
  # try making dataframe and going that route. need correct ids for match
  # between then and now note <- cellsize could be unsolveable. Add rotation of
  # grid if needed.
...
calculate_grid.sf <- function(
  ...
  calculate_grid(as(shape, "Spatial"), learning_rate = learning_rate,
  grid_type = grid_type, seed = seed, verbose = verbose)
}

#' Calculate size of grid items (deprecated).
#'
#' Given an input multipolgyon spatial data frame this function calculates the
#' required cell size of a regular or hexagonal grid.
#' @param shape A 'SpatialPolygonsDataFrame' object representing the original
#' spatial polygons.
...
#' @param seed An optional random seed integer to be used for the grid
#' calculation algorithm.
#' @param verbose A logical indicating whether messages should be printed as the
#' algorithm iterates.
#' @importFrom sp spsample HexPoints2SpatialPolygons SpatialPixels
#' @importFrom methods as
#' @export
calculate_cell_size <- function(
  shape, shape_details = NULL, learning_rate = 0.03,
  grid_type = c("hexagonal", "regular"), seed = NULL, verbose = FALSE
) {
