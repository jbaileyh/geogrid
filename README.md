...
par(mfrow = c(2, 3), mar = c(0, 0, 2, 0))
for (i in 1:6) {
  new_cells <- calculate_cell_size(shape = original_shapes, grid_type = "regular", seed = i)
...
``` r
new_cells_hex <- calculate_cell_size(shape = original_shapes, grid_type = "hexagonal", seed = 3)
resulthex <- assign_polygons(original_shapes, new_cells_hex)

new_cells_reg <- calculate_cell_size(shape = original_shapes, grid_type = "regular", seed = 3)
...
2.  `calculate_cell_size()` given your input polygons this will generate the grid as specified by your arguments:
...
3.  `assign_polygons()`: this will assign the original polygons to their new locations on the grid generated in `calculate_cell_size()`. It will find the solution that minimises the sum of the total distance between the original polygon centroids and eventual gridded centroids. Arguments:
    -   `shape`: the original polygons from `read_polygons()`
...
``` r
new_cells_hex <- calculate_cell_size(shape = original_shapes, grid_type = "hexagonal", seed = 6)
resulthex <- assign_polygons(original_shapes, new_cells_hex)

new_cells_reg <- calculate_cell_size(shape = original_shapes, grid_type = "regular", seed = 4)
resultreg <- assign_polygons(original_shapes, new_cells_reg)
...
new_cells_reg <- calculate_cell_size(shape = original_shapes, grid_type = "regular", seed = 1)
resultreg <- assign_polygons(original_shapes, new_cells_reg)
