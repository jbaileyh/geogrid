...
shape_dts <- get_shape_details(input_shape = shapes)

grids <- calculate_cell_size(shape = shapes, 
                             shape_details = shape_dts,
                             learning_rate = 0.03,
                             grid_type = 'regular')

result <- assign_polygons(shape = shapes, 
                          new_polygons = grids, 
                          method = "hungarian")
```
...
#Generate colours for the plotting
rbpal <- colorRampPalette(viridis(20))
colors1 = rbpal(20)[as.numeric(cut(shapes$CENSUSAREA,breaks = 10))]
colors2 = rbpal(20)[as.numeric(cut(result$CENSUSAREA,breaks = 10))]

par(mfrow=c(2,1), mar=c(1,1,1,1),bg = "black")
plot(shapes, col = colors1, border = NA, main = "Polygon area (ha)")
text(coordinates(shapes), labels = substr(shapes$NAME,1,4), cex = 0.5, col = "white")
plot(result, col = colors2, border = NA, sub = "Method = smallest first")
text(coordinates(result), labels = substr(result$NAME,1,4), cex = 0.5, col = "white")
```
